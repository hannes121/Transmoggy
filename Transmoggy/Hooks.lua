...
-- Possible extra lines compared to SetHyperlink
local valid = {
...
local Tooltip_AddCollectedLine = function(tooltip, itemID, link)
	...
	-- Try to insert our line directly under blizzards lines
	local blizzNumLines = BlizzNumLines(tooltip, link or ("item:" .. itemID))
	...
	if not tooltip.collectedText then	
		...
		tooltip.collectedText:SetJustifyV("MIDDLE")
	...
	if text then
		...
		tooltip.collectedText:SetWidth(math.min(tooltip.collectedText:GetStringWidth(), tooltip:GetWidth() - 15)) -- Line break works/looks better than forcing tooltip to text's width imo

		tooltip.collectedAnchor = _G[tooltip:GetName() .. "TextLeft" .. blizzNumLines]
		tooltip.collectedAnchor:SetJustifyV("TOP")
		-- tooltip.collectedAnchor:SetWidth(math.max(tooltip.collectedText:GetStringWidth(), tooltip:GetWidth() - 15))
...
local function OnTooltipSetItem(tooltip)
	tooltip:Show() -- resize incase other AddOns changed line widths and did not do this
	...
	if not tooltip.mogText then	
		...
		tooltip.HideTransmogLine = function(self)
			if textLeft1.justifyHOld then
				textLeft1:SetHeight(0)
				textLeft1:SetWidth(0)
	...
	textLeft1:SetWidth(math.max(tooltip.mogText:GetStringWidth(), tooltip:GetWidth() - 10))
	...
	Tooltip_AddCollectedLine(tooltip, itemID, link)
end
...
-- Or add collected line here, so we easily know the original lines (+ OnTooltipSetSpell)
-- for _, f in pairs({"SetBagItem", "SetInventoryItem", "SetHyperlink", "SetHyperlinkCompareItem", "SetGuildBankItem", "SetBuybackItem", "SetInboxItem"}) do
-- 	hooksecurefunc(GameTooltip, f, function(self, ...) print(f, ...) end)
-- end

-- Add both to AtlasLootTooltip
local hookAtlasLoot = true -- TODO: setting needed?
if hookAtlasLoot then
	if AtlasLootTooltip then
		core.HookItemTooltip(AtlasLootTooltip)
		AtlasLootTooltip:HookScript("OnTooltipSetSpell", OnTooltipSetSpell)
	else
		local f = CreateFrame("Frame")
		f:RegisterEvent("ADDON_LOADED")
		f:SetScript("OnEvent", function(self, event, name)
			if event == "ADDON_LOADED" and name == "AtlasLoot" then
...
core.PreHook_ModifiedItemClick = function()
	...
	HandleModifiedItemClick = function(link, ...)
		...
		if IsModifiedClick("DRESSUP") and IsModifiedClick("CHATLINK") then
			...
			-- Itemlinks do not contain the enchant's spellID itself, but the ID for the [visual] effect
			-- Mapping back from effect to spell is not always unique, so DressUp might not know the right spellID
			local enchantID = core.SpellToEnchantID(illusionID) or core.GetEnchantIDFromLink(link) or 0
			
			-- Construct link
			if visualID == core.HIDDEN_ID then
				-- We do not have "hidden belt" items like blizzard
				-- So unless we call the slot-extended dressup function directly, we can not set a slot to hidden with this
				link = core.HIDDEN
			else
				if visualID ~= core.UNMOG_ID then
					local _
					_, link = GetItemInfo(visualID)
				end
				if enchantID and enchantID ~= 0 then
					link = link and link:gsub("|Hitem:(%d+):(%d*):", "|Hitem:%1:" .. enchantID .. ":")
				end
			end
		end
