...
local stringDataVersion = 1.1 -- 1.1: now with checksum to detect wtf file bytestring corruption
...
local SimpleChecksum = function(str)
	local s = 0
    for i = 1, #str do
...
core.GenerateStringData = function()
	...
	if TransmoggyDB.stringData
			and stringDataVersion == TransmoggyDB.stringDataVersion
			and TransmoggyDB.stringDataIsCompressed == useCompression
			and TransmoggyDB.stringDataLocale == GetLocale() then
		-- also check if our bytestrings somehow got corrupted (e.g. by user manually editing the wtf file)
		local checkSum = SimpleChecksum(TransmoggyDB.stringDataPos or "")
		if checkSum == TransmoggyDB.stringDataChecksum then
			...
			core.Debug("Corrupted stringData in cache. Checksum:", checkSum, "(should be", TransmoggyDB.stringDataChecksum, ")")
		end
	end
	...
	TransmoggyDB.stringDataChecksum = SimpleChecksum(core.stringDataPos)
