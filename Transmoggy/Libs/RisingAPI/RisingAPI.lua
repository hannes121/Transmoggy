...
function RisingAPI:_handleServerResponse(id, message, done)
	...
	local request = self._clientRequests[id]
	if not request then
		logger:error(LOG_ERROR, "invalid request id (request does not exist, might be caused by a previous request timeout)")
		return
	end
...
RisingAPI._events = RisingAPI._events or Utils.CreateEventRegistry(
	function(event)
		RisingAPI:request("event/register", { event = event }):catch(function(err)
			logger:error(LOG_ERROR, "failed to register for event \"%s\": %s", event, err.message)
		end)
	end,
	function(event)
		RisingAPI:request("event/unregister", { event = event }):catch(function(err)
			logger:error(LOG_ERROR, "failed to unregister from event \"%s\": %s", event, err.message)
		end)
	end
)
