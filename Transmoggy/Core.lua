...
core.TUTORIAL_QUEST_ID = 110440
local locale = GetLocale()
core.TUTORIAL_QUEST_NAME = locale == "deDE" and "Transmogrifizieren" or "Transmogrify"
...
core.sounds = {
	...
	unlockVisual = 6347, -- 1202 (put down leather), 1204 (put down gem), 1519 (taxinode), 12009 (forest troll male aggro), 6347 (wisp yes)
	gainBalance = 1210, -- 1210 (pick up ring), 1204 (put down gem), 120 (loot window coin sound)
}

core.soundFiles = {
	applySuccess = "Sound\\Spells\\ShaysBell.wav",
	unlockVisual = "Sound\\Event Sounds\\Wisp\\WispYes2.wav",
	gainBalance = "Sound\\interface\\PickUp\\PutDownRing.wav",
}
...
local OnVisualUnlocked = function(payload)
	core.Debug("OnVisualUnlocked", payload)
	...
	if core.db and core.db.profile.General.playSpecialSounds then
		PlaySoundFile(core.soundFiles.unlockVisual, "master")
	end
...
local OnBalanceChanged = function(payload)
	...
	if core.db and core.db.profile.General.playSpecialSounds then
		...
		if payload and payload.shards and balance and balance.shards and payload.shards > balance.shards then
			PlaySoundFile(core.soundFiles.gainBalance, "master")
		end
	end
...
core.RequestUnlocksSlot = function(slot)
	...
	local isEnchantSlot = core.IsEnchantSlot(slot)
	local itemID = isEnchantSlot and core.TransmogGetSlotInfo(core.GetCorrespondingSlot(slot)) or core.TransmogGetSlotInfo(slot)
	local enchantID = isEnchantSlot and core.TransmogGetSlotInfo(slot) or nil
	...
	f(unpack(p)):next(function(answer)
		if requestID == requestCounterUS[transmogLocation] then
			local items = isEnchantSlot and answer.spellIds or answer.itemIds
			-- Add currently equiped item/enchant to the list (highlighted as current item)
			...
			if not skin then
				if isEnchantSlot and enchantID then
					table.insert(items, enchantID)
				elseif not isEnchantSlot and itemID then
...
core.RequestApplyCurrentChanges = function()
	...
	API.ApplyAll(core.ToApiSet(requestedChanges, true), skinID):next(function(answer)
		if requestID == requestCounterACC then
			PlaySoundFile(core.soundFiles.applySuccess, "master")
...
core.tutorialQuestCompleted = false
...
a:SetScript("OnEvent", function(self, event, ...)
	...
	elseif event == "GOSSIP_SHOW" then
		...
		-- local tutorialQuestCompleted = GetNumGossipOptions() > 10
		local tutorialQuestCompleted = core.tutorialQuestCompleted

		core.SetShown(core.gossipOpenTransmogButton, isTransmogNPC and tutorialQuestCompleted)

		if isTransmogNPC and tutorialQuestCompleted and core.db.profile.General.autoOpen then
...
questTracker:RegisterEvent("QUEST_QUERY_COMPLETE")
questTracker:RegisterEvent("CHAT_MSG_SYSTEM")
questTracker:SetScript("OnEvent", function(self, event, ...)
	...
	elseif event == "QUEST_QUERY_COMPLETE" then
		local tmp = {}
		GetQuestsCompleted(tmp)
		core.tutorialQuestCompleted = tmp[core.TUTORIAL_QUEST_ID]

	elseif event == "CHAT_MSG_SYSTEM" then
		local msg = ...
		local locale = GetLocale()
		local pattern = locale == "deDE" and "^(.-) abgeschlossen%.$" or "^(.-) completed%.$"
		local questName = string.match(msg, pattern)

		if questName == core.TUTORIAL_QUEST_NAME then
			core.tutorialQuestCompleted = true
		end
	end

	if core.tutorialQuestCompleted then
