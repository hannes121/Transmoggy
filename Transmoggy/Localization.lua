...
local title = GetAddOnMetadata(folder, "Title") or ""
local version = GetAddOnMetadata(folder, "Version") or ""
local author = GetAddOnMetadata(folder, "Author") or ""
local email = GetAddOnMetadata(folder, "X-Email") or ""
local website = GetAddOnMetadata(folder, "X-Website") or ""
...
L.deDE = {
    ...
    ABOUT_HEADER_NAME = string.format("%s %s", title, version),
    ...
    ABOUT_MADE_BY_NAME = string.format("Entwickelt von %s. Die aktuelle Version findet ihr unter |cfffff0b0%s|r. Für Feedback oder Anfragen zum AddOn bin ich auch unter |cfffff0b0%s|r erreichbar.", author, website, email),
}
...
L.enEU = {
    ...
    APPEARANCE_TOOLTIP_TEXT4 = "* Using this item for transmogrification will bind it to you and make it non-tradable and non-refundable.",
    ...
    ABOUT_MADE_BY_NAME = string.format("Made by %s. The most recent version can be found under |cfffff0b0%s|r. You can report issues in the github or send feedback or requests regarding the AddOn to |cfffff0b0%s|r.", author, website, email),
}
